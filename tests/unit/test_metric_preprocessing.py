...
def test_add_container_ids_missing_column():
    ...
    vocab = build_container_vocabulary(
        pd.DataFrame({"container_labels": ["container=webapp,pod=test"]})
    )
...
def test_metric_preprocessor_multi_container():
    ...
    df = pd.DataFrame(
        {
            "timestamp": pd.date_range("2024-01-01", periods=300, freq="15s").tolist()
            * 3,
            "container_labels": (
...
def test_metric_preprocessor_outlier_handling():
    ...
    preprocessor = MetricPreprocessor(
        MetricType.CPU, config={"outlier_threshold": 2.0}  # 2 standard deviations
    )
