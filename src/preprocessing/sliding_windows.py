...
class TimeSeriesWindowGenerator:
    ...
    def create_sequences(
        self, data: np.ndarray, timestamps: Optional[pd.DatetimeIndex] = None
...
def prepare_container_metrics(
    df: pd.DataFrame,
    container_name: str,
    metric_name: str = "container_cpu",
    resample_freq: str = "15S",
) -> pd.DataFrame:
    """
...
def add_lag_features(
    df: pd.DataFrame, value_column: str = "value", lags: List[int] = [1, 4, 12, 24]
) -> pd.DataFrame:
    """
...
def add_rolling_features(
    df: pd.DataFrame, value_column: str = "value", windows: List[int] = [4, 12, 24, 60]
) -> pd.DataFrame:
    """
...
def create_features_and_windows(
    df: pd.DataFrame,
    ...
    feature_columns = [
        col for col in prepared.columns if col not in ["timestamp", "value"]
    ]
...
def create_multi_horizon_features_and_windows(
    ...
    window_size = window_size_minutes * 4  # 4 intervals per minute
    prediction_horizons = [
        h * 4 for h in prediction_horizon_minutes
...
if __name__ == "__main__":
    ...
    if data_files:
        ...
        X_multi, y_multi_dict, features_multi, meta_multi = (
