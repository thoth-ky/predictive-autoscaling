...
def baseline_load(duration_seconds=3600, rps=10):
    ...
    print(
        f"Running baseline load: {rps} req/s for {duration_seconds}s targeting {BASE_URL}"
    )
    ...
    with ThreadPoolExecutor(max_workers=20) as executor:
        while time.time() < end_time:
            ...
            endpoint = random.choices(
                ["light", "medium", "heavy", "error"], weights=[50, 30, 15, 5]
            )[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "pattern",
        choices=["baseline", "spike", "periodic", "gradual", "chaos", "full"],
