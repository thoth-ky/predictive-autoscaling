...
class TemporalSplitter:
    ...
    def split(self,
              ...
              metadata: Optional[np.ndarray] = None) -> Tuple:
        """
...
def split_temporal_data(X: np.ndarray,
                       y: Union[np.ndarray, Dict[int, np.ndarray]],
                       train_ratio: float = 0.7,
                       val_ratio: float = 0.15,
                       test_ratio: float = 0.15,
...
if __name__ == '__main__':
    ...
    # Perform split
    X_train, X_val, X_test, y_train_dict, y_val_dict, y_test_dict = splitter.split(X, y_dict)
    ...
    print(f"  Val samples: indices {split_info['train_samples']} to {split_info['val_indices'][1]-1}")
